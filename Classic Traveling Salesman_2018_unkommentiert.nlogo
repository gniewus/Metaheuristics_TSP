...
turtles-own [
  string    ;;
  pdistance    ;;The fitness and the pdistance are the same. Pdistance is put in to keep things from getting too confusing
  fitness
  last-patch-x
  last-patch-y
  edge-table
...
to go
  ...
  if ticks = 0
   [set global-min-fitness 1000
  set start-time date-and-time]
...
to calculate-distance
    set pdistance 0
    set last-patch-x 0
    set last-patch-y 0 ;;The following commands may seem like a lot, but really they repeat over and over. Shown here is how the algorithm interprets the list of numbers

    setxy 0 0 set pdistance 0 set last-patch-x p-0-x set last-patch-y p-0-y
    ...
    if item item# string = 20 [setxy p-20-x p-20-y set pdistance (pdistance) + distancexy-nowrap last-patch-x last-patch-y set last-patch-x p-20-x set last-patch-y p-20-y]
     ...
     set item# item# + 1                                                                          ;;last-patch-x equals the last 'x' cordnate that the turtle was at
...
to calculate-fitness
    ...
    set looser max-one-of turtles [fitness]
end
